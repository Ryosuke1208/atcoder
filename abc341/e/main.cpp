...
struct ST {
    ...
    struct T {
        ...
        // make sure the return type matches the actual type of the returned
        // value
        bool res() { return isGood; }
    };
    ...
    ST(int n) {
        this->n = n;
        this->h = 31 - __countl_zero(unsigned(n));
        t.resize(n << 1);
        ln.resize(n << 1);
        d.resize(n);
   ...
   private:
    void calc(int i) { t[i] = t[i << 1] + t[i << 1 | 1]; }
